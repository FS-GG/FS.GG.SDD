...
- **Markdown projections** (`summary.md`, `commands.md`, `skills.md`) are
  ...
  version** (`2.0.2-preview.2`, from `generatorVersion.version` in the envelope) and their
...
| `summary.md` | generated view (`summary`) / Markdown projection | gen (`2.0.2-preview.2`) | — | AdditiveOptional | byte-stable | `readiness/<id>/summary.md` |
...
| `agent-commands/<target>/commands.md` | generated view (`agentCommands`) / Markdown projection | gen (`2.0.2-preview.2`) | — | AdditiveOptional | byte-stable | `readiness/<id>/agent-commands/<target>/commands.md` |
| `agent-commands/<target>/skills.md` | generated view (`agentCommands`) / Markdown projection | gen (`2.0.2-preview.2`) | — | AdditiveOptional | byte-stable | `readiness/<id>/agent-commands/<target>/skills.md` |
...
> `contractVersion` (`2.0.0`), which moves independently of its `schemaVersion`.
...
> `1.2.0` adds `performanceEvidence[]`, carrying raw bound samples and SDD's
> reproducible percentile/catch-up calculations instead of a verdict summary.
> `2.0.0` adds the canonical optional `performanceEvidence[].intent`; it is required for newly
> governed profiles and null for legacy/non-interactive evidence.
...
- **`work-model.json`** — `decisions`, `diagnostics`, `evidence`, `performanceIntent`,
  ...
  `performanceBudget.{artifactPath,intent,targetFps,workloadIds,stressWorkloadIds,workloadDefinitionDigests,currencyToken,capturedAfterUtc,maxP95Ms,maxP99Ms,maxCatchUpFrames,measurementScope,requiredCapability,liveCompositorRequired,deferralIssue}`.
