...
sourced from `Directory.Build.props` `<Version>` — currently **`0.10.0`**:
...
number, so `release-readiness.json` carries `identity.version = "0.10.0"` and
`generatorVersion.version = "0.10.0"`. A consumer can therefore determine the
