...
sourced from `Directory.Build.props` `<Version>` — currently **`0.2.1`**:
...
number, so `release-readiness.json` carries `identity.version = "0.2.1"` and
`generatorVersion.version = "0.2.1"`. A consumer can therefore determine the
