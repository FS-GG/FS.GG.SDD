...
sourced from `Directory.Build.local.props` `<Version>` — currently **`1.7.0`**:
...
number, so `release-readiness.json` carries `identity.version = "1.7.0"` and
`generatorVersion.version = "1.7.0"`. A consumer can therefore determine the
