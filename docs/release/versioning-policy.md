...
sourced from `Directory.Build.props` `<Version>` — currently **`0.8.0`**:
...
number, so `release-readiness.json` carries `identity.version = "0.8.0"` and
`generatorVersion.version = "0.8.0"`. A consumer can therefore determine the
