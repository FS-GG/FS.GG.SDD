    ...
    type ScaffoldSummary =
        {
            ...
            /// `pinned` (SDD created it), `merged` (owned entries were added without removing
            /// co-tenants), `current` (owned entries were already exact), `failed` (write,
            /// malformed-state, or owned-entry conflict), or `notApplicable` (the step never
            /// ran: dry run, provider defect, or a pre-invocation block).
