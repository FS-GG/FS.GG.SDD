...
let viewRoots = bareRoots kitRootDeclaration.["FsggKitViewSkillRoots"] |> Set.ofList
...
for view in Set.toList viewRoots |> List.sort do
    ...
    let disposition =
        if not (Directory.Exists dir) then
            "not generated in this tree"
        else
            let info = DirectoryInfo dir

            if info.LinkTarget <> null then
                sprintf "generated, symlink -> %s" info.LinkTarget
            else
                "generated, copy-mode (a real directory)"
...
printfn
    "  write set    : %s"
    (if List.isEmpty writeRoots then
         "EMPTY — every declared root is the source or a generated view"
     else
         String.concat " " writeRoots)
