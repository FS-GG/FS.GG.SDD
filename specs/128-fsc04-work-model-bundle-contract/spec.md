...
The next physical adapter must treat a performance artifact as an explicitly selected file rather than assert that its arbitrary parent is a closed producer root. It needs descriptor-pinned, no-follow traversal through every ancestor; regular-file and byte-stability checks on the opened descriptor; and a defined response to case aliases, link or path replacement, and parent roster mutation. Its result must bind the exact selected path and bytes to the pure bundle comparison. This draft does not implement that adapter and cannot claim physical closure, atomic cross-root capture, or installed parity.
