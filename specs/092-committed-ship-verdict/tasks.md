...
- [ ] T001 Build and test on the untouched branch; confirm green and that the lock file is untouched.
- [ ] T002 Reproduce research D1 in a scratch git repository: with `readiness/*/` +
...
- [ ] T003 [P] Reproduce research D2: confirm `ArtifactTaxonomyTests.fs:68`'s
...
- [ ] T004 [P] Inventory what must move: every enumeration of the generated-view set —
...
- [ ] T005 [P] Record the golden `ship.json`'s expected `sourcesDigest`
...
- [ ] T006 [US1] Extend `ShipView` with `DispositionBlockingFindingIds: string list` in
...
- [ ] T007 [US1] Add `GeneratedViewKind.ShipVerdict` to
...
- [ ] T008 [US1] Author `src/FS.GG.SDD.Artifacts/LifecycleArtifacts/ShipVerdict.fsi` — exactly
...
- [ ] T009 [US1] Implement `ShipVerdict.fs`: `sourcesDigest` as the path-sorted
...
- [ ] T010 [P] [US1] `tests/FS.GG.SDD.Artifacts.Tests/ShipVerdictTests.fs` — NEW. `fromShipView` copies
...
- [ ] T011 [P] [US1] Same file — the ship-ready shape is exactly 20 lines (FR-004); a verdict with
      *n* blocking finding ids is `20 + n` lines.
- [ ] T012 [P] [US1] Same file — `sourcesDigest` equals the value recomputed independently in the test
...
- [ ] T013 [P] [US1] `tests/FS.GG.SDD.Artifacts.Tests/ShipViewTests.fs` — `parseShipView` now surfaces
      `disposition.blockingFindingIds`, sorted; a `ship.json` without the field parses to `[]`.
- [ ] T014 [US1] `tests/FS.GG.SDD.Commands.Tests/ShipCommandTests.fs` — `ship` writes
...
- [ ] T015 [US1] `tests/FS.GG.SDD.Commands.Tests/goldens/readiness/ship-verdict.json` — NEW golden, and
      a `ReadinessViewGoldenTests.fs` case asserting the emitted bytes match it.

### Implementation

- [ ] T016 [US1] `Foundation.fs` — add `shipVerdictPath` beside `shipPath`; add
      `ReadFile(shipVerdictPath workId)` to `refreshReadEffects` so `refresh` can snapshot it.
- [ ] T017 [US1] `HandlersShip.fs` — add the pure `shipVerdictEmission workId generator shipText`
...
- [ ] T018 [US1] `HandlersShip.fs` — append its effects and view state beside the existing `ship.json`
      write, **inside** the current `not hasBlocking` gate, so FR-005 holds with no new branch.
- [ ] T019 [P] [US1] `LifecycleArtifacts/Core.fs` — add the `ship-verdict.json` artifact-inventory row
...
- [ ] T020 [P] [US3] `tests/FS.GG.SDD.Commands.Tests/RefreshCommandTests.fs` — after `ship`, an
      unchanged `refresh` leaves the verdict byte-identical and reports it among
      `AlreadyCurrentViewIds` (FR-006 / SC-006).
- [ ] T021 [P] [US3] Same file — a deleted verdict with a current `ship.json` is restored by `refresh`
      and reported `Refreshed`.
- [ ] T022 [P] [US3] Same file — with `ship.json` stale / missing / malformed / blocked, the verdict
      **inherits** that class and is not rewritten (FR-006, second half).
- [ ] T023 [P] [US3] Same file — the bytes `ship` writes and the bytes `refresh` writes are identical
      for the same inputs (FR-007 / SC-006).

### Implementation

- [ ] T024 [US3] `HandlersRefresh.fs` — re-project the verdict in the `governance-handoff` slot, gated
...
- [ ] T025 [US3] `HandlersRefresh.fs` — add `"ship-verdict"` to `refreshCanonicalViews` and to every
      per-view bucket list (`perViewState`, `classifyToBucket`, the blocked-path `perViewState`), so it
      is reported like its siblings.
- [ ] T026 [P] [US3] `ValidationHarness.fs` — add `"ship-verdict.json"` to `determinismOutputs` and fix
...
- [ ] T027 [P] [US3] `tests/FS.GG.SDD.Validation.Tests/DeterminismMatrixTests.fs` — extend
      `DeterminismOutputs`; confirm `CoverageGapTests` reconciles against the real surface.
...
- [ ] T028 [US2] `tests/FS.GG.SDD.Commands.Tests/GitignoreNegationTests.fs` — NEW. In a temp dir:
...
- [ ] T029 [US2] Same file — the **regression**: with the pre-feature `readiness/*/` rule written into
...
- [ ] T030 [P] [US2] Same file — `git check-ignore` reports every nested
      `agent-commands/<target>/…` path ignored, and `ship-verdict.json` not ignored (SC-004).
- [ ] T031 [P] [US2] Same file — this repository's own rule: in a temp repo carrying the repo's
...
- [ ] T032 [P] [US2] `ArtifactTaxonomyTests.fs` — strengthen the vacuous assert (T003): assert the
...
- [ ] T033 [US2] `Foundation.fs` `gitignoreSeedText` — `readiness/*/` → `readiness/*/*` +
...
- [ ] T034 [US2] The repository's own `.gitignore` — `specs/*/readiness/` →
      `specs/*/readiness/*/*` + `!specs/*/readiness/*/ship-verdict.json`. Root `readiness/<id>/`
      pinned proofs stay matched by no rule (FR-010).
- [ ] T035 [US2] Confirm `Drift.fs` needs **no** change (presence-only, research D2) and that
...
- [ ] T036 [P] [US4] `ArtifactTaxonomyTests.fs` — re-partition the guard: regenerable block ≡
...
- [ ] T037 [P] [US4] `ReleaseBoundaryTests.fs` — amend T024's `known` set to admit `ShipVerdict`, and
      rename/comment it to record that feature 092 introduced the kind (FR-016).
- [ ] T038 [P] [US4] `tests/FS.GG.SDD.Artifacts.Tests/ReleaseContractTests.fs` — `durableGenerated`
      round-trips through `serialize`/`parse`; it is `true` for the verdict and `false` for every other
      entry (FR-012 / SC-009).
- [ ] T039 [P] [US4] `ReleaseConformanceTests.fs` — add `ship-verdict.json` to `producedArtifacts`;
      T015's catalog ≡ produced-set equality holds exactly.
- [ ] T040 [P] [US4] `ReleaseReadinessCheckTests.fs` — T019 (catalog covers every enumerable kind) still
      passes, and fails if the catalog entry is removed while the kind exists (SC-010).

### Implementation

- [ ] T041 [US4] `ReleaseContract.fsi`/`.fs` — add `SchemaReferenceEntry.DurableGenerated: bool`;
...
- [ ] T042 [US4] `ReleaseContract.fs` `currentRelease()` — add the `ship-verdict.json` catalog entry
...
- [ ] T043 [US4] Regenerate `docs/release/release-readiness.json` and
...
- [ ] T044 [US4] `docs/reference/artifact-taxonomy.md` — add the **durable generated** table (with
...
- [ ] T045 [P] [US4] `docs/release/schema-reference.md` — add the catalog table row and the JSON field
...
- [ ] T046 Regenerate `tests/FS.GG.SDD.Artifacts.Tests/PublicSurface.baseline` with
...
- [ ] T047 [P] Update the three `fs-gg-sdd-ship` skill mirrors (`.claude/`, `.codex/`, `.agents/`) — the
...
- [ ] T048 [P] Run `fsgg-sdd validate --text` end-to-end; confirm `ship-verdict.json` is enumerated and
      no `CoverageGap` is reported (FR-017 / SC-011).
- [ ] T049 Confirm `ship.json` is byte-identical to `main`'s on the same fixture, no
      `registry/dependencies.yml` entry was added, and no `contractVersion` introduced (FR-018 / SC-012).
- [ ] T050 Full `dotnet build` + `dotnet test` green; `git status --short -- '*packages.lock.json'`
