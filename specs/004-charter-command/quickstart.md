# Quickstart: Charter Command

## Purpose

This guide describes the validation path for the `fsgg-sdd charter` feature.
...
- An initialized project can create `work/<id>/charter.md`.
- The charter has valid structured front matter and standard sections.
- Successful reports identify `specify` as the next lifecycle action.
- Reruns preserve authored prose.
- Safe missing-section additions are reported precisely.
- Identity mismatches, malformed front matter, missing project settings, and
  unsafe overwrites block before authored writes.
...
- Charter reads project and charter snapshots before planning writes.
...
- Charter reports `readiness/<id>/work-model.json` state.
...
## Prove Deterministic Charter Reports
...
- Three dry-run charter executions over identical fixtures produce
  byte-identical JSON.
- Reports contain no timestamps, absolute host paths, ANSI output, process ids,
  random ids, or nondeterministic ordering.
...
- Charter works when Governance files are absent.
...
- SDD does not select routes, evaluate freshness, adjust profiles, select
  gates, emit protected-boundary verdicts, or evaluate release policy.
...
- The prelude can construct a `charter` command request.
...
dotnet run --project src/FS.GG.SDD.Cli -- charter --root "$tmp_dir" --work 004-charter-command --title "Charter Command" --json
dotnet run --project src/FS.GG.SDD.Cli -- charter --root "$tmp_dir" --work 004-charter-command --title "Charter Command" --dry-run --json
dotnet run --project src/FS.GG.SDD.Cli -- charter --root "$tmp_dir" --work 004-charter-command --title "Charter Command" --text
```

Expected outcome:

- `init` creates the minimum SDD skeleton without Governance.
- `charter` creates `work/004-charter-command/charter.md`.
- Dry-run rerun does not mutate disk and reports no unsafe overwrite.
- Text mode reflects the same changed artifacts, generated-view state,
  diagnostics, and next action as JSON.
- All JSON reports follow [contracts/charter-report-json.md](contracts/charter-report-json.md).
...
- Charter tests add coverage without introducing Governance runtime
  requirements.
