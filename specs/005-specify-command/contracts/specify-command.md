# Contract: `fsgg-sdd specify`

## Scope

`fsgg-sdd specify` creates or safely updates the work-item specification after
the work item has a valid charter. The command emits the same deterministic
...
```bash
fsgg-sdd specify --root <project-root> --work <work-id> --input <specification-intent> [--title <title>] [--dry-run] [--text]
```
...
`--input` is required for new specifications. Existing complete
specifications may be rerun without input and report `noChange` when safe.
...
- `work/<id>/charter.md` exists and has valid charter front matter for the
  selected work id.
- New specifications receive enough intent to seed user value, scope, and at
  least one measurable requirement.
- Existing `work/<id>/spec.md`, when present, has readable content and
  matching specification identity.
...
- Missing or mismatched charter prerequisite blocks before spec writes.
- Missing intent for a new specification blocks before spec writes.
- Existing specification identity mismatch blocks before writes.
...
4. Load selected charter, existing specification, later lifecycle source
   snapshots, existing work-model view, and duplicate-id candidates.
5. Validate charter prerequisite.
6. Normalize specification intent.
7. Build the proposed specification artifact or safe update plan.
...
- Creates or updates `work/<id>/spec.md`.
- Records `stage: specify` in specification front matter.
- Preserves existing authored prose and stable ids.
- Lists the specification artifact in `changedArtifacts`.
...
- Sets next action to `clarify` for the selected work id.
...
- Leaves existing specification content unchanged.
...
or blocked specify request after project/work-id validation.
...
- `fsgg-sdd clarify`, `checklist`, `plan`, `tasks`, or `analyze` behavior;
