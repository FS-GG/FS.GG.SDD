...
- [ ] T022 [US1] `src/FS.GG.SDD.Commands/CommandReports/ReportAssembly.fs`: bump `ReportVersion` `1.3.0 → 1.4.0`; add the changelog comment line; add `VersionBump` to the empty model in `helpReport`.
...
- [ ] T028 [US3] `SurfaceCommandTests.fs`: V19 — `--param versionAxisFile=../outside.props` ⇒ `undeterminable`, **and no `ReadFile` is planned for it** (FR-017). Assert on the planned effect set.
...
- [ ] T038 [P] `docs/release/schema-reference.md`: describe the `surface.versionBump` block (this is exactly what 087 did for `classification`). Confirm `docs/release/release-readiness.json` and its test baseline stay **untouched** — the `inventory` list names only top-level report blocks (R9).
...
- [ ] T040 Read `tests/FS.GG.SDD.Commands.Tests/RemediationSupport.fs` and confirm `RemediationPointersTests` gates **blocking** diagnostics only. If it also gates warnings, add the remediation pointer for `surface.versionBumpRequired`. (Do not assume — plan.md flags this as unverified.)
...
- [ ] T043 `fsgg-sdd validate --markdown` — the cross-cutting harness exercises `surface` × three projections × representative states. Confirm no new non-passing cell.
- [ ] T044 Dogfood: run `fsgg-sdd surface --check --param versionAxisFile=Directory.Build.local.props` **in this repo** and confirm it resolves `0.8.0`. Note that this repo keeps a reflection baseline rather than a `docs/api-surface/` tree, so the classification will be `none` and no warning fires — that is the correct, inert result (US4-2, spec §Overview).
