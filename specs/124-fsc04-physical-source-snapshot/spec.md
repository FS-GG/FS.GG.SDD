...
- FR-003: Read each opened Linux regular-file descriptor twice, compare the byte passes and supported metadata before, between, and after them, then return defensive byte copies with a digest under the explicitly selected exact-byte or UTF-8 LF-text policy. Refuse observed in-place mutation. A later file mutation changes a later capture, not the prior snapshot.
...
For each opened regular file, Linux compares two byte passes and checks supported inode, size, mtime, and ctime fields from that same descriptor before the first pass, between passes, and after the second pass. A controlled same-length overwrite after the first pass now refuses. This remains an observed-stability check: writes after the last check, ABA or timestamp-hidden mutation, and cross-file or cross-root simultaneous consistency are not proved. The producer's work-model multi-root source selection and transform still require a separate contract join.
