...
2. **Given** that same run, **When** the resulting `plan.md` is compared to its pre-run content, **Then** the only difference is inside the `## Source Snapshot` section body — `## Plan Scope`, `## Plan Decisions`, `## Contract Impact`, `## Verification Obligations`, `## Migration Posture`, `## Generated View Impact`, `## Accepted Deferrals`, `## Planning Findings`, and `## Advisory Notes` are byte-identical, and the front matter is byte-identical.
...
- **FR-004**: `fsgg-sdd plan --accept-upstream` MUST rewrite the body of the `## Source Snapshot` section from the current sources and MUST leave the front matter and every other section byte-identical.
...
- **SC-002**: Across the full lifecycle test matrix, `fsgg-sdd` writes **zero** tool-synthesized lines into `work/<id>/plan.md`'s authored sections; a byte-level test asserts that no `plan` invocation on an existing plan alters any section other than `## Source Snapshot`, and then only under `--accept-upstream`.
