...
- **FR-004**: `fsgg-sdd plan --accept-upstream` MUST rewrite the body of the `## Source Snapshot` section from the current sources **whenever the flag is passed** — not only when the recorded digests differ. Gating the rewrite on staleness makes an *empty* snapshot unrecoverable (nothing recorded ⇒ nothing changed ⇒ no rewrite ⇒ still empty), permanently disabling FR-008/FR-016 for that plan. It MUST NOT synthesize any `PD-###` line. It MAY append the derived entries `plan` already appends for genuinely-new upstream ids (`plannedPlanEntries`/`appendPlanEntries`, which diff against the existing plan facts) — that is pre-existing, intended behavior and not part of this defect. It MUST leave the front matter and all authored prose otherwise untouched.
...
- **FR-008**: `fsgg-sdd tasks` and `fsgg-sdd analyze` — and **only** those two — MUST detect a stale `plan.md` snapshot from the digests and block with the same `stalePlanSnapshot` `DiagnosticError`, independent of any `stale:` marker in the plan's prose. They are the stages that *derive* from the plan. `evidence`, `verify`, and `ship` read the same shared `PlanDiagnostics` list but MUST NOT block on it: a prose typo in `spec.md` must never brick the back half of the lifecycle by withholding `readiness/<id>/verify.json` and `ship.json`, nor route the operator to re-run `plan` at ship time (which would append derived rows to an implemented plan, out of lifecycle order).
...
- **FR-010**: Every `stalePlanSnapshot` diagnostic MUST carry a remediation pointing at `fsgg-sdd plan --accept-upstream`, surfaced through the existing next-action routing as the `plan.acceptUpstream` action. That action MUST report the **full** blocking-diagnostic set, not just `stalePlanSnapshot`: it fires whenever the snapshot is stale, which may be alongside an unrelated blocker, and an agent driving off `blockingDiagnosticIds` would otherwise loop once per hidden blocker.
- **FR-011**: A successful `fsgg-sdd plan` MUST emit exactly one non-blocking advisory stating which sources it has snapshotted and that later edits to them require `fsgg-sdd plan --accept-upstream`. The advisory MUST NOT change the exit code, the outcome, or `changedArtifacts`, and MUST NOT be emitted on any **blocked** run — including a blocked *creation*, where no `plan.md` is written at all and the advisory would assert a snapshot that does not exist.
...
- **FR-016**: A recorded snapshot entry with an absent digest MUST NOT be treated as stale, preserving today's `sourceDigestsStale` semantics so that pre-existing plans do not become blocked on upgrade. `plan --accept-upstream` MUST still be able to establish digests for such a plan (see FR-004).
- **FR-017**: `stalePlanSnapshot` MUST classify as `staleSource` in the analysis-view finding taxonomy — a source moved under a derived artifact — so `analysis.json` reports it as "re-derive from current sources" rather than "the artifacts are wrong".
