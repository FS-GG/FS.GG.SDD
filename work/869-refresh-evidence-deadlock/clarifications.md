...
- **DEC-004** [CQ-001] [FR-003] [AC-003]: No gate is deleted, and the coverage that says so carries a CONTROL leg. Measured, the three commands do not all do the same thing: `evidence` SEEDS the undeclared obligation and names it (that is DEC-005, and asserting a refusal there would assert the opposite of what this change intends), `verify` refuses naming each id, and `ship` refuses behind the verification view `verify` declined to produce. The control leg is the load-bearing part: the obvious fixture for this property never runs `analyze`, so `verify` refuses at `evidence.missingAnalysisPrerequisite` whether the declarations are present or absent, and a test asserting only the outcome would pass against a build with the enforcement deleted outright.
