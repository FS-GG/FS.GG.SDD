...
- AC-003 [US-003] [FR-003]: Given an evidence obligation that is never declared, when `evidence`, `verify` and `ship` run, then each still refuses and names the obligation, so no enforcement is lost by AC-001.
...
- FR-003: `evidence`, `verify` and `ship` continue to refuse an evidence obligation that carries no declaration and no accepted deferral, naming the obligation id. (Stories: US-003; Acceptance: AC-003)
