...
- AC-005 [US-002] [FR-004]: Given a Git archive/source projection has no repository metadata, or the Git authority command cannot return an exact-candidate answer, when evidence, verification, and shipping are evaluated, then `shipReady` is impossible and the report identifies the rejected path and unavailable authority.
...
- FR-005: The implementation MUST include inversion controls for ignored-existing, untracked-existing, staged-only, missing-on-clean-checkout, Git-archive/no-repository, unavailable-Git-command, tracked-work-evidence positive, and explicit-external-receipt positive cases. (covers AC-001, AC-002, AC-003, AC-004, AC-005)
