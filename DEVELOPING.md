...
- F# `.fsi` signature files for submodules under `CommandReports/` and
  `CommandWorkflow/` follow a two-tier convention by module visibility, not per-file
  taste:
  - A **public** submodule (`namespace FS.GG.SDD.Commands`, `module <Name>`) that is
    consumed across the assembly boundary — by `Cli` or a test project — carries its
    own paired `.fsi` to pin and document that public surface (today:
    `LifecycleFooter`, `LintEngine`, `ProcessSkillManifest`).
  - An **internal** implementation submodule (`namespace FS.GG.SDD.Commands.Internal`,
    ...
    `CommandWorkflow.fsi`. Adding a per-file `.fsi` here is redundant.

  So a new submodule gets a `.fsi` iff it is a public, cross-assembly module.
