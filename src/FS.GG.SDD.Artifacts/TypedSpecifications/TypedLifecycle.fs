...
module TypedAuthorityManifest =
    let path (workId: string) =
        $"readiness/{workId}/typed-authority.json"
        ...
        try
            ...
            let required (name: string) =
                ...
                | true, value when value.ValueKind = JsonValueKind.String ->
                    value.GetString()
                    |> Option.ofObj
                    |> Option.defaultWith (fun () -> raise (FormatException($"Null string field '{name}'.")))
            ...
            if schemaVersion <> 1 then
                Error(
                    diagnostic
                        ...
                        $"Authority schema {schemaVersion} is unsupported."
                        "Upgrade the CLI or regenerate with schemaVersion 1."
                )
            else
                Ok
    ...
    let markdownProjection (workId: string) model =
        SpecificationProjection.generate RequirementsExtension.contract model
        |> Result.map (fun projection ->
            let title = JsonSerializer.Serialize model.Intent

            let body =
                projection.Markdown
                    .Replace("## User value", "## User Value")
                    .Replace("## Non-goals", "## Non-Goals")
                    .Replace("## User stories", "## User Stories")
                    .Replace("## Acceptance criteria", "## Acceptance Scenarios")
                    .Replace("## Requirements", "## Functional Requirements")
                    .Replace("## Public impact", "## Public Or Tool-Facing Impact")
                    .Replace("## Lifecycle notes", "## Lifecycle Notes")

            let publicImpact =
                string (not (List.isEmpty model.Extension.PublicImpact))
    ...
    let validateDerivation (canonicalBytes: byte array) (normalizedBytes: byte array) (markdownBytes: byte array) =
        ...
        let canonical = Encoding.UTF8.GetString canonicalBytes
            ...
            if finish < 0 then
                ...
                | Error findings ->
                    ...
                    |> List.map (fun finding ->
                        diagnostic "typedSdd.canonicalMalformed" finding.Message "Correct the canonical typed model.")
                | Ok model ->
                    let workId =
                        model.Provenance.SourcePath.Split('/')
                        |> Array.tryItem 1
                        |> Option.defaultValue "unknown"

                    match
                        SpecificationCodec.serialize RequirementsExtension.contract model,
                        markdownProjection workId model
                    with
                    | Ok expectedNormalized, Ok markdown ->
