...
module private BindingV2Internal =
    ...
    and listSource values = values |> List.map valueSource |> listText
    ...
    let source (moduleName: string) (fingerprint: string) (canonicalJson: string) (contract: QuintCompiledContractV2) =
        ...
        builder.Append("[<Literal>]\nlet Schema = ").AppendLine(BindingInternal.escapeString contract.Schema) |> ignore
        builder.Append("[<Literal>]\nlet Profile = ").AppendLine(BindingInternal.escapeString contract.Profile) |> ignore
        builder.Append("[<Literal>]\nlet Specification = ").AppendLine(BindingInternal.escapeString contract.Specification) |> ignore
        builder.Append("[<Literal>]\nlet ContractFingerprint = ").AppendLine(BindingInternal.escapeString fingerprint) |> ignore
        builder.Append("[<Literal>]\nlet CanonicalContractJson = ").AppendLine(BindingInternal.escapeString canonicalJson) |> ignore
        builder
            .AppendLine()
...
module QuintBindingsV2 =
    let generate moduleName contract =
        ...
        let bindingDiagnostics =
            [ if String.IsNullOrWhiteSpace moduleName || BindingInternal.identifier moduleName <> moduleName then
                  yield BindingInternal.diagnostic "QBD-MODULE-NAME" "$.moduleName" "Module name must already be one generated PascalCase identifier."
              for generated, rows in ids |> List.groupBy BindingInternal.identifier |> List.sortBy fst do
                  if String.IsNullOrEmpty generated then
                      yield BindingInternal.diagnostic "QBD-IDENTIFIER-EMPTY" "$.catalogue" "An identity cannot generate an F# identifier."
                  elif rows.Length > 1 then
                      yield BindingInternal.diagnostic "QBD-IDENTIFIER-COLLISION" "$.catalogue" $"Identities collide as generated identifier '%s{generated}'." ]
