...
module private Portable =
    ...
    let frameBytes (bytes: byte array) =
        Array.concat [ int32 bytes.Length; bytes ]
    ...
    let validate (contract: ExtensionContract<'extension>) (model: SpecificationModel<'extension>) =
        ...
        [ if model.SchemaVersion <> 1 then
          ...
          if not (lowercaseHex model.Provenance.SourceRevision) then
              yield
                  diagnostic
          ...
          yield! contract.Validate model.EvidenceObligations model.Extension ]
        |> sort
    ...
    let sha256 (bytes: byte array) =
        ...
        for start in 0..64 .. padded.Length - 64 do
            ...
            for i in 16..63 do
                ...
                w[i] <-
                    w[i - 16]
                    + (rotate 7 a ^^^ rotate 18 a ^^^ (a >>> 3))
                    + w[i - 7]
                    + (rotate 17 b ^^^ rotate 19 b ^^^ (b >>> 10))

            let mutable a = hash[0] in
            let mutable b = hash[1] in
            let mutable c = hash[2] in
            let mutable d = hash[3]
            let mutable e = hash[4] in
            let mutable f = hash[5] in
            let mutable g = hash[6] in
            ...
            for i in 0..63 do
                ...
                let t2 =
                    (rotate 2 a ^^^ rotate 13 a ^^^ rotate 22 a)
                    + ((a &&& b) ^^^ (a &&& c) ^^^ (b &&& c))
    ...
    let digest bytes =
        sha256 bytes |> Array.map (fun b -> b.ToString("x2")) |> String.concat ""
...
module SpecificationCompiler =
    ...
    let normalize contract model =
        ...
        | [] -> Ok(Portable.normalizedBytes contract model)
        | errors -> Error errors

    let fingerprint contract model =
        normalize contract model |> Result.map Portable.digest
        ...
        | Ok bytes ->
            Ok
                { Model = model
                  ...
                  Fingerprint = Portable.digest bytes }
    ...
    let semanticDiff contract before after =
        ...
        if not diagnostics.IsEmpty then
            ...
            let evidence model =
                Portable.evidenceBytes model.EvidenceObligations |> digest
            ...
            let changes =
                [ if before.Identity <> after.Identity then
                  ...
                  if extension before <> extension after then
                      yield
                          { Path = "/extension"
                            Summary = "Typed specification extension changed."
