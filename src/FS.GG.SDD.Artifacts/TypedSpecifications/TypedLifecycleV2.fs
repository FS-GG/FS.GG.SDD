                                ...
                                | Ok source, Ok fenceManifest, Ok sourceMap ->
                                    ...
                                    let extracted =
                                        try
                                            ...
                                            |> List.map (fun fence ->
                                                let contentLines =
                                                    sourceLines[fence.SourceRange.Start.Line .. fence.SourceRange
                                                                                                    .End.Line
                                                                                                - 2]
                                    ...
                                    then
                                        ...
                                        let extractedModule =
                                            extracted
                                            |> Result.map (
                                                List.map (fun (_, content, _) -> content) >> String.concat ""
                                            )
    ...
    let private semanticClosureGeneral observations (manifest: QuintAuthorityManifest) =
        ...
        [
            ...
            | Some contractText, Some selectorText, Some typedEffectText, Some bindingText ->
                match
                    QuintContractV2.deserialize contractText, QuintGeneralBindingManifest.deserialize selectorText
                with
