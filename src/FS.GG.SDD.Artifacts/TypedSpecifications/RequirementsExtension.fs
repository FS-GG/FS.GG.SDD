...
module RequirementsDraft =
    ...
    let addScope boundary (draft: RequirementsDraft) = { draft with Scope = boundary :: draft.Scope }
    let addNonGoal boundary (draft: RequirementsDraft) = { draft with NonGoals = boundary :: draft.NonGoals }
    let addStory story (draft: RequirementsDraft) = { draft with Stories = story :: draft.Stories }
    let addRequirement requirement (draft: RequirementsDraft) = { draft with Requirements = requirement :: draft.Requirements }
    let addAcceptance acceptance (draft: RequirementsDraft) = { draft with Acceptance = acceptance :: draft.Acceptance }
    let addAmbiguity ambiguity (draft: RequirementsDraft) = { draft with Ambiguities = ambiguity :: draft.Ambiguities }
    let addPublicImpact impact (draft: RequirementsDraft) = { draft with PublicImpact = impact :: draft.PublicImpact }
    let addLifecycleNote note (draft: RequirementsDraft) = { draft with LifecycleNotes = note :: draft.LifecycleNotes }
...
module private Requirements =
    ...
    let sortedIds (ids: SpecificationId list) = ids |> List.distinct |> List.sortBy idText
    ...
    let duplicates path ids =
        ...
        |> List.choose (fun (identifier, count) ->
            if count > 1 then
                Some(diagnostic "REQ-ID-DUPLICATE" path $"Identifier '%s{idText identifier}' is declared more than once.")
            else
                None)
    ...
    let validateWithEvidence (obligations: EvidenceObligation list) (extension: RequirementsExtension) =
        ...
        let allIds = scopeIds @ nonGoalIds @ storyIds @ requirementIds @ acceptanceIds @ ambiguityIds
        ...
        [ yield! blank "REQ-USER-VALUE-REQUIRED" "/extension/userValue" "User value" extension.UserValue
          ...
          for index, boundary in extension.Scope |> List.indexed do
              yield! blank "REQ-SCOPE-STATEMENT-REQUIRED" $"/extension/scope/%d{index}/statement" "Scope statement" boundary.Statement

          for index, boundary in extension.NonGoals |> List.indexed do
              yield! blank "REQ-NON-GOAL-STATEMENT-REQUIRED" $"/extension/nonGoals/%d{index}/statement" "Non-goal statement" boundary.Statement

          for index, story in extension.Stories |> List.indexed do
              yield! blank "REQ-STORY-PRIORITY-REQUIRED" $"/extension/stories/%d{index}/priority" "Story priority" story.Priority
              yield! blank "REQ-STORY-STATEMENT-REQUIRED" $"/extension/stories/%d{index}/statement" "Story statement" story.Statement

          for index, requirement in extension.Requirements |> List.indexed do
              yield! blank "REQ-STATEMENT-REQUIRED" $"/extension/requirements/%d{index}/statement" "Requirement statement" requirement.Statement
              yield! duplicateReferences "REQ-ACCEPTANCE-DUPLICATE" $"/extension/requirements/%d{index}/acceptanceIds" requirement.AcceptanceIds
              yield! duplicateReferences "REQ-EVIDENCE-DUPLICATE" $"/extension/requirements/%d{index}/evidenceObligationIds" requirement.EvidenceObligationIds
              if List.isEmpty requirement.AcceptanceIds then
                  yield diagnostic "REQ-ACCEPTANCE-REQUIRED" $"/extension/requirements/%d{index}/acceptanceIds" "Requirement must reference at least one acceptance criterion."
              for referenced in requirement.AcceptanceIds do
                  if not (Set.contains referenced acceptance) then
                      yield diagnostic "REQ-ACCEPTANCE-UNRESOLVED" $"/extension/requirements/%d{index}/acceptanceIds" $"Acceptance reference '%s{idText referenced}' does not resolve."
              for referenced in requirement.EvidenceObligationIds do
                  if not (Set.contains referenced evidence) then
                      yield diagnostic "REQ-EVIDENCE-UNRESOLVED" $"/extension/requirements/%d{index}/evidenceObligationIds" $"Evidence reference '%s{idText referenced}' does not resolve."

          for index, criterion in extension.Acceptance |> List.indexed do
              yield! blank "REQ-ACCEPTANCE-STATEMENT-REQUIRED" $"/extension/acceptance/%d{index}/statement" "Acceptance statement" criterion.Statement
              yield! duplicateReferences "REQ-STORY-REFERENCE-DUPLICATE" $"/extension/acceptance/%d{index}/storyIds" criterion.StoryIds
              yield! duplicateReferences "REQ-REQUIREMENT-REFERENCE-DUPLICATE" $"/extension/acceptance/%d{index}/requirementIds" criterion.RequirementIds
              for referenced in criterion.StoryIds do
                  if not (Set.contains referenced stories) then
                      yield diagnostic "REQ-STORY-UNRESOLVED" $"/extension/acceptance/%d{index}/storyIds" $"Story reference '%s{idText referenced}' does not resolve."
              for referenced in criterion.RequirementIds do
                  if not (Set.contains referenced requirements) then
                      yield diagnostic "REQ-REQUIREMENT-UNRESOLVED" $"/extension/acceptance/%d{index}/requirementIds" $"Requirement reference '%s{idText referenced}' does not resolve."

          for index, ambiguity in extension.Ambiguities |> List.indexed do
              yield! blank "REQ-AMBIGUITY-QUESTION-REQUIRED" $"/extension/ambiguities/%d{index}/question" "Ambiguity question" ambiguity.Question
              match ambiguity.State, ambiguity.Decision with
              | Open, None -> ()
              | Open, Some _ ->
                  yield diagnostic "REQ-AMBIGUITY-OPEN-DECISION" $"/extension/ambiguities/%d{index}/decision" "Open ambiguity cannot carry a resolved decision."
              | (Resolved | Deferred), Some decision when not (String.IsNullOrWhiteSpace decision) -> ()
              | Resolved, _ ->
                  yield diagnostic "REQ-AMBIGUITY-DECISION-REQUIRED" $"/extension/ambiguities/%d{index}/decision" "Resolved ambiguity requires a decision."
              | Deferred, _ ->
                  yield diagnostic "REQ-AMBIGUITY-DEFERRAL-REQUIRED" $"/extension/ambiguities/%d{index}/decision" "Deferred ambiguity requires a visible deferral decision."
    ...
    let stateName = function Open -> "open" | Resolved -> "resolved" | Deferred -> "deferred"
    ...
    let parseIdArray name element =
        ...
        |> Result.bind (fun values ->
            ...
            |> List.map (fun value ->
                ...
                else Error $"Array '%s{name}' must contain strings.")
            |> fun results ->
                let errors = results |> List.choose (function Error error -> Some error | _ -> None)
                if List.isEmpty errors then Ok(results |> List.choose (function Ok value -> Some value | _ -> None))
        ...
        try
            ...
            let version =
                ...
                | Some value when value.ValueKind = JsonValueKind.Number ->
                    match value.TryGetInt32() with true, number -> Ok number | _ -> Error "schemaVersion must be an integer."
                | _ -> Error "schemaVersion must be an integer."

            let boundaries name =
                parseList name (parseRecord "id" (fun identifier item ->
                    stringProperty "statement" item |> Result.map (fun statement -> { Id = identifier; Statement = statement }))) element

            let stories =
                parseList "stories" (parseRecord "id" (fun identifier item ->
                    match stringProperty "priority" item, stringProperty "statement" item with
                    | Ok priority, Ok statement -> Ok { Id = identifier; Priority = priority; Statement = statement }
                    | Error error, _ | _, Error error -> Error error)) element

            let requirements =
                parseList "requirements" (parseRecord "id" (fun identifier item ->
                    match stringProperty "statement" item, parseIdArray "acceptanceIds" item, parseIdArray "evidenceObligationIds" item with
            ...
            let acceptance =
                parseList "acceptance" (parseRecord "id" (fun identifier item ->
                    match parseIdArray "storyIds" item, parseIdArray "requirementIds" item, stringProperty "statement" item with
            ...
            let ambiguities =
                parseList "ambiguities" (parseRecord "id" (fun identifier item ->
                    ...
                    | Ok question, Ok stateText ->
                        ...
                        |> Result.map (fun state ->
                            let decision =
                                ...
                                | _ -> None
                            { Id = identifier; Question = question; State = state; Decision = decision })
                    | Error error, _ | _, Error error -> Error error)) element

            let strings name =
                arrayProperty name element
                |> Result.bind (fun values ->
                    if values |> List.forall (fun item -> item.ValueKind = JsonValueKind.String && not (isNull (item.GetString()))) then
                        Ok(values |> List.choose (fun item -> item.GetString() |> Option.ofObj))
                    else
                        Error $"Array '%s{name}' must contain strings.")

            match
                schema, version, stringProperty "userValue" element, boundaries "scope", boundaries "nonGoals",
                stories, requirements, acceptance, ambiguities, strings "publicImpact", strings "lifecycleNotes"
            with
            | Ok schemaValue, Ok versionValue, Ok userValue, Ok scope, Ok nonGoals,
              Ok storyRows, Ok requirementRows, Ok acceptanceRows, Ok ambiguityRows, Ok publicImpact, Ok lifecycleNotes
                when schemaValue = "fsgg.requirements-extension/v1" && versionValue = 1 ->
            ...
            | values ->
                let message =
                    [ match values with Error e, _, _, _, _, _, _, _, _, _, _ -> yield e | _ -> ()
                      match values with _, Error e, _, _, _, _, _, _, _, _, _ -> yield e | _ -> ()
                      match values with _, _, Error e, _, _, _, _, _, _, _, _ -> yield e | _ -> ()
                      match values with _, _, _, Error e, _, _, _, _, _, _, _ -> yield e | _ -> ()
                      match values with _, _, _, _, Error e, _, _, _, _, _, _ -> yield e | _ -> ()
                      match values with _, _, _, _, _, Error e, _, _, _, _, _ -> yield e | _ -> ()
                      match values with _, _, _, _, _, _, Error e, _, _, _, _ -> yield e | _ -> ()
                      match values with _, _, _, _, _, _, _, Error e, _, _, _ -> yield e | _ -> ()
                      match values with _, _, _, _, _, _, _, _, Error e, _, _ -> yield e | _ -> ()
                      match values with _, _, _, _, _, _, _, _, _, Error e, _ -> yield e | _ -> ()
                      match values with _, _, _, _, _, _, _, _, _, _, Error e -> yield e | _ -> ()
                      match schema, version with
                      | Ok schemaValue, Ok versionValue when schemaValue <> "fsgg.requirements-extension/v1" || versionValue <> 1 ->
    ...
    let markdown (extension: RequirementsExtension) =
        ...
        let acceptanceRows =
            ...
            |> List.map (fun item ->
                let tags = (item.StoryIds @ item.RequirementIds) |> sortedIds |> List.map (fun id -> $"[%s{idText id}]") |> String.concat " "
        ...
        let ambiguityRows =
            ...
            |> List.map (fun item ->
                let decision = item.Decision |> Option.map (fun value -> " — " + value) |> Option.defaultValue ""
        ...
        @ ambiguityRows
        @ [ ""; "## Public impact"; "" ]
        @ (if List.isEmpty extension.PublicImpact then [ "- None." ] else extension.PublicImpact |> List.sort |> List.map (fun value -> "- " + value))
        @ [ ""; "## Lifecycle notes"; "" ]
        @ (if List.isEmpty extension.LifecycleNotes then [ "- None." ] else extension.LifecycleNotes |> List.sort |> List.map (fun value -> "- " + value))

[<RequireQualifiedAccess>]
module RequirementsExtension =
    let validate (extension: RequirementsExtension) = Requirements.validateWithEvidence [] extension
...
module RequirementsMigration =
    ...
    let private scopeRow = Regex(@"^-\s+(SB-\d{3,}):\s+(.+)$", RegexOptions.Compiled ||| RegexOptions.IgnoreCase)
    let private storyRow = Regex(@"^-\s+(US-\d{3,})\s+\(([^)]+)\):\s+(.+)$", RegexOptions.Compiled ||| RegexOptions.IgnoreCase)
    let private acceptanceRow = Regex(@"^-\s+(AC-\d{3,})\s+(.+?):\s+(.+)$", RegexOptions.Compiled ||| RegexOptions.IgnoreCase)
    let private requirementRow = Regex(@"^-\s+(FR-\d{3,}):\s+(.+)$", RegexOptions.Compiled ||| RegexOptions.IgnoreCase)
    let private ambiguityRow = Regex(@"^-\s+(AMB-\d{3,})(?:\s+(open|resolved|deferred))?:\s+(.+)$", RegexOptions.Compiled ||| RegexOptions.IgnoreCase)
    let private idReference = Regex(@"\b(?:US|FR|AC|EV)\-\d{3,}\b", RegexOptions.Compiled ||| RegexOptions.IgnoreCase)
    ...
    let analyzeMarkdown (markdown: string) =
        ...
        let supported =
            set
                [ "User Value"; "Scope"; "Non-Goals"; "User Stories"; "Acceptance Scenarios"
                  "Functional Requirements"; "Ambiguities"; "Public Or Tool-Facing Impact"; "Lifecycle Notes" ]
        ...
        for index, line in lines |> Array.indexed do
            ...
            if matched.Success then
                ...
                if not (Set.contains current supported) then
                    unknown.Add(finding "REQ-MIGRATION-UNKNOWN-HEADING" UnknownSemanticHeading $"Heading '%s{current}' has no P2 requirements representation." (index + 1) 1)
        ...
        let schemaLine =
            lines
            |> Array.indexed
            |> Array.tryFind (fun (_, line) -> line.TrimStart().StartsWith("schemaVersion:", StringComparison.OrdinalIgnoreCase))

        let schemaFindings =
            match schemaLine with
            | Some(index, line) when (line.Split([| ':' |], 2)[1]).Trim() = "1" -> []
            | Some(index, _) -> [ finding "REQ-MIGRATION-SCHEMA" UnsupportedSchemaVersion "Only Standard SDD schemaVersion 1 can migrate." (index + 1) 1 ]
            | None -> [ finding "REQ-MIGRATION-SCHEMA" UnsupportedSchemaVersion "Standard SDD schemaVersion is required." 1 1 ]

        let nonEmptyUnknown =
            unknown
            |> Seq.filter (fun item ->
                let headingName = lines[item.Location.Line - 1] |> fun value -> heading.Match(value).Groups[1].Value
        ...
        let parseBoundaries section =
            ...
            |> List.map (fun (line, text) ->
                ...
                if matched.Success then
                    ...
                    |> Result.map (fun id -> { Id = id; Statement = matched.Groups[2].Value })
                else
                    Error(finding "REQ-MIGRATION-SCOPE" MalformedConstruct $"%s{section} row is not a stable SB-### list item." line 1))
        ...
        let parsedStories =
            ...
            |> List.map (fun (line, text) ->
                ...
                if matched.Success then
                    ...
                    Error(finding "REQ-MIGRATION-STORY" MalformedConstruct "User story is not a stable US-### (priority) list item." line 1))
        ...
        let parsedAcceptance =
            ...
            |> List.map (fun (line, text) ->
                ...
                if matched.Success then
                    let references = idReference.Matches(text) |> Seq.cast<Match> |> Seq.map _.Value.ToUpperInvariant() |> List.ofSeq
                    match identifier matched.Groups[1].Value line with
                    | Error error -> Error error
                    | Ok id ->
                        let ids = references |> List.choose (fun value -> SpecificationId.create value |> Result.toOption)
                        Ok
                            { Id = id
                              StoryIds = ids |> List.filter (SpecificationId.value >> _.StartsWith("US-", StringComparison.Ordinal))
                              RequirementIds = ids |> List.filter (SpecificationId.value >> _.StartsWith("FR-", StringComparison.Ordinal))
                              Statement = matched.Groups[3].Value }
                else
                    Error(finding "REQ-MIGRATION-ACCEPTANCE" MalformedConstruct "Acceptance scenario is not a stable AC-### list item." line 1))
                    ...
                    | Ok id ->
                        ...
                        Ok
                            { Id = id
                              ...
                              AcceptanceIds = ids |> List.filter (SpecificationId.value >> _.StartsWith("AC-", StringComparison.Ordinal))
                              EvidenceObligationIds = ids |> List.filter (SpecificationId.value >> _.StartsWith("EV", StringComparison.Ordinal)) }
                else
                    Error(finding "REQ-MIGRATION-REQUIREMENT" MalformedConstruct "Requirement is not a stable FR-### list item." line 1))
            ...
            |> List.map (fun (line, text) ->
                ...
                if matched.Success then
                    ...
                    | Ok id ->
                        ...
                        Ok
                            { Id = id
                              Question = matched.Groups[3].Value
                              State = state
                              Decision = None }
                else
                    Error(finding "REQ-MIGRATION-AMBIGUITY" MalformedConstruct "Ambiguity is not a stable AMB-### list item." line 1))

        let errors results = results |> List.choose (function Error error -> Some error | _ -> None)
        let parseFindings =
            errors parsedScope
        ...
        let missingRequired =
            [ if List.isEmpty valueRows then
              ...
              if List.isEmpty (sectionRows sections "Acceptance Scenarios" |> meaningful) then
                  yield finding "REQ-MIGRATION-ACCEPTANCE" MalformedConstruct "Acceptance Scenarios section is required." 1 1
              if List.isEmpty (sectionRows sections "Functional Requirements" |> meaningful) then
                  yield finding "REQ-MIGRATION-REQUIREMENT" MalformedConstruct "Functional Requirements section is required." 1 1 ]

        let unsupported = schemaFindings @ nonEmptyUnknown @ parseFindings @ missingRequired

        if not (List.isEmpty unsupported) then
            Unsupported(unsupported |> List.sortBy (fun item -> item.Location.Line, item.Location.Column, item.Code))
        else
            let take results = results |> List.choose (function Ok value -> Some value | _ -> None)
            let extension: RequirementsExtension =
                { UserValue = valueRows |> List.map snd |> String.concat "\n"
                  ...
                  PublicImpact = sectionRows sections "Public Or Tool-Facing Impact" |> meaningful |> List.map (snd >> _.TrimStart('-', ' '))
                  LifecycleNotes = sectionRows sections "Lifecycle Notes" |> meaningful |> List.map (snd >> _.TrimStart('-', ' ')) }
            ...
            let unresolved =
                ...
                |> List.map (fun item ->
                    ...
                    let idText = if referenced.Success then referenced.Groups[1].Value else ""
                    let line =
                        lines
            ...
            if List.isEmpty unresolved then Migrated extension
            else Ambiguous(unresolved |> List.sortBy (fun item -> item.Location.Line, item.Code))
