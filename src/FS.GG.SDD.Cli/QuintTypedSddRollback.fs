...
module internal QuintTypedSddRollback =
    ...
    let private decode (bytes: byte array) =
        try
            ...
            if
                Set.ofList names <> set [ "schema"; "entries" ]
                || names.Length <> 2
            ...
            then
                ...
                |> Seq.map (fun item ->
                    ...
                    if
                        Set.ofList itemNames <> set [ "originalPath"; "backupPath"; "sha256"; "bytes" ]
                        || itemNames.Length <> 4
                    then
    ...
    let snapshot rootPath workId sourceRelative =
        try
            ...
            let authorityPath =
                containedPath rootPath authorityRelative
                |> Option.defaultWith (fun () -> invalidOp "unsafe authority path")
                    ...
                    | Ok(FsharpSpecificationV1 authority) ->
                        let read relative =
                            let path =
                                ...
                                |> Option.defaultWith (fun () -> invalidOp "unsafe v1 authority path")

                            if not (File.Exists path) then
                                invalidOp $"v1 authority path '{relative}' is missing"
            ...
            let entries, backupWrites =
                ...
                |> List.mapi (fun index original ->
                    let full =
                        containedPath rootPath original
                        |> Option.defaultWith (fun () -> invalidOp $"unsafe rollback source '{original}'")

                    if not (File.Exists full) then
                        invalidOp $"rollback source '{original}' is missing"
    ...
    let restore rootPath workId (authority: QuintAuthorityManifest) apply =
        ...
        | Some inventoryRelative, Some inventorySha ->
            try
                let inventoryPath =
                    containedPath rootPath inventoryRelative
                    |> Option.defaultWith (fun () -> invalidOp "unsafe rollback inventory path")

                let inventoryBytes = File.ReadAllBytes inventoryPath

                if TypedAuthorityManifest.sha256 inventoryBytes <> inventorySha then
                    invalidOp "rollback inventory digest mismatch"

                let entries = decode inventoryBytes |> Result.defaultWith invalidOp

                if
                    List.isEmpty entries
                    || (entries |> List.map _.OriginalPath |> List.distinct |> List.length)
                       <> entries.Length
                then
                ...
                let restores =
                    ...
                    |> List.map (fun entry ->
                        let original =
                            containedPath rootPath entry.OriginalPath
                            |> Option.defaultWith (fun () -> invalidOp "unsafe original path")

                        let backup =
                            containedPath rootPath entry.BackupPath
                            |> Option.defaultWith (fun () -> invalidOp "unsafe backup path")

                        let bytes = File.ReadAllBytes backup

                        if
                            int64 bytes.Length <> entry.Bytes
                            || TypedAuthorityManifest.sha256 bytes <> entry.Sha256
                        then
                ...
                let v2Relatives =
                    TypedAuthorityManifest.path workId :: (authority.Artifacts |> List.map _.Path)

                let backupRelatives = inventoryRelative :: (entries |> List.map _.BackupPath)

                let affectedRelatives =
                    (v2Relatives @ backupRelatives @ (entries |> List.map _.OriginalPath))
                    |> List.distinct
                ...
                let deletes =
                    ...
                    |> List.map (fun relative ->
                        containedPath rootPath relative
                        |> Option.defaultWith (fun () -> invalidOp "unsafe delete path"))
                ...
                try
                    ...
                    for entry in entries do
                        ...
                        if
                            ...
                            || TypedAuthorityManifest.sha256 (File.ReadAllBytes path) <> entry.Sha256
                        then
