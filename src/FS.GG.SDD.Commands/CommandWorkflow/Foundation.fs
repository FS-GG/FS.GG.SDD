...
"""
    ...
    // re-included per feature). Carries the same no-clobber `AgentGuidanceTarget` write-kind as
    // the constitution/early-stage guidance, so an existing product `.gitignore` is never
    // clobbered (existing repos adopt the fragment from docs/reference/artifact-taxonomy.md).
    // Generic build/tooling outputs are toolchain-generic, not provider-specific. Exact bytes are
    // pinned by a drift guard (spec 073 FR-005); keep the taxonomy doc's fragment in sync.
    let gitignoreSeedText =
...
"""
    ...
    /// Feature 092 / ADR-0026: the committed merge-boundary verdict.
