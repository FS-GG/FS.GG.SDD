...
module CommandHelp =
    ...
    let globalFlags =
        [ flag "--root" (Some "<path>") "Project root directory (default: the current directory)."
          ...
          flag "--rich" None "Emit human-oriented rich output; degrades to plain text when non-interactive or color-disabled."
          flag "--help, -h" None "Show usage and flags for the command." ]

    let commandEntries =
        [ { Name = "init"; Description = "Seed the SDD skeleton in the current directory." }
          { Name = "charter"; Description = "Create the work item charter." }
          { Name = "specify"; Description = "Draft the specification from labeled intent." }
          { Name = "clarify"; Description = "Record clarification questions, answers, and decisions." }
          { Name = "checklist"; Description = "Review requirements quality before planning." }
          { Name = "plan"; Description = "Author the implementation plan." }
          { Name = "tasks"; Description = "Generate the dependency-ordered task breakdown." }
          { Name = "analyze"; Description = "Analyze cross-artifact lifecycle readiness." }
          { Name = "evidence"; Description = "Declare authored verification evidence." }
          { Name = "verify"; Description = "Evaluate verification readiness and emit verify.json." }
          { Name = "ship"; Description = "Aggregate merge-boundary ship readiness and emit ship.json." }
    ...
    let private input = flag "--input" (Some "<text>") "Labeled intent lines used to seed the artifact."
    let private dryRun = flag "--dry-run" None "Report proposed changes without writing."
