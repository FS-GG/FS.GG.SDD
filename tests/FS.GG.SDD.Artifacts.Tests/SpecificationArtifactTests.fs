...
"""
    ...
    let ``Specification scoped ids validate expected shapes`` () =
        Assert.Equal("US-001", Identifiers.createUserStoryId "us-001" |> Result.map Identifiers.userStoryIdValue |> Result.defaultWith failwith)
        Assert.Equal("AC-001", Identifiers.createAcceptanceScenarioId "ac-001" |> Result.map Identifiers.acceptanceScenarioIdValue |> Result.defaultWith failwith)
        Assert.Equal("SB-001", Identifiers.createScopeBoundaryId "sb-001" |> Result.map Identifiers.scopeBoundaryIdValue |> Result.defaultWith failwith)
        Assert.Equal("AMB-001", Identifiers.createAmbiguityId "amb-001" |> Result.map Identifiers.ambiguityIdValue |> Result.defaultWith failwith)
    ...
    let ``Specification parser extracts front matter sections ids and references`` () =
        ...
        | Ok facts ->
            ...
            Assert.Equal<string list>([ "AC-001" ], facts.AcceptanceScenarioIds |> List.map Identifiers.acceptanceScenarioIdValue)
            Assert.Equal<string list>([ "SB-001"; "SB-002" ], facts.ScopeBoundaryIds |> List.map Identifiers.scopeBoundaryIdValue)
