...
/// build's generator version (0.25.1); fixtures declare minimums above/at/below it.
...
module ScaffoldCliCoherenceTests =
    ...
    // US2 scenario 1 (SC-002): installed 0.25.1 < declared 0.26.0 ⇒ exactly one
    ...
    // 0.26.0 > 0.25.1 numerically (components are ints, not strings) — see Fsgg.Version.
    ...
    let ``behind minimum emits exactly one cliBehindMinimum advisory naming installed minimum and gap`` () =
        ...
        | [ diagnostic ] ->
            ...
            Assert.Contains("0.26.0", diagnostic.Message)
            Assert.Contains("0.25.1", diagnostic.Message)
