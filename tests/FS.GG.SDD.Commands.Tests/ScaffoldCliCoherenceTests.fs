...
module ScaffoldCliCoherenceTests =
    ...
    // US2 scenario 1 (SC-002): installed 0.29.0 < declared 0.30.0 ⇒ exactly one
    ...
    // 0.30.0 > 0.29.0 numerically (components are ints, not strings) — see Fsgg.Version.
    ...
    let ``behind minimum emits exactly one cliBehindMinimum advisory naming installed minimum and gap`` () =
        ...
        | [ diagnostic ] ->
            ...
            Assert.Contains("0.29.0", diagnostic.Message)
