...
/// build's generator version (0.10.0); fixtures declare minimums above/at/below it.
...
module ScaffoldCliCoherenceTests =
    ...
    // US2 scenario 1 (SC-002): installed 0.10.0 < declared 0.11.0 ⇒ exactly one
    ...
    // 0.11.0 > 0.10.0 numerically (components are ints, not strings) — see Fsgg.Version.
    ...
    let ``unparseable installed version skips the comparison and records the CLI version honestly`` () =
        ...
        // and does NOT persist the malformed *minimum* — it is a valid minimum here (0.11.0),
