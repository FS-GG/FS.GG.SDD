...
/// build's generator version (0.6.0); fixtures declare minimums above/at/below it.
...
module ScaffoldCliCoherenceTests =
    ...
    // US2 scenario 1 (SC-002): installed 0.6.0 < declared 0.7.0 ⇒ exactly one
    ...
    let ``unparseable installed version skips the comparison and records the CLI version honestly`` () =
        ...
        // and does NOT persist the malformed *minimum* — it is a valid minimum here (0.7.0),
