...
/// build's generator version (0.23.0); fixtures declare minimums above/at/below it.
...
module ScaffoldCliCoherenceTests =
    ...
    // US2 scenario 1 (SC-002): installed 0.23.0 < declared 0.24.0 ⇒ exactly one
    ...
    // 0.24.0 > 0.23.0 numerically (components are ints, not strings) — see Fsgg.Version.
    ...
    let ``behind minimum emits exactly one cliBehindMinimum advisory naming installed minimum and gap`` () =
        ...
        | [ diagnostic ] ->
            ...
            Assert.Contains("0.23.0", diagnostic.Message)
            Assert.Contains("0.24.0", diagnostic.Message)
