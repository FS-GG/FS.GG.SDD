...
/// build's generator version (0.15.0); fixtures declare minimums above/at/below it.
...
module ScaffoldCliCoherenceTests =
    ...
    // US2 scenario 1 (SC-002): installed 0.15.0 < declared 0.16.0 ⇒ exactly one
    ...
    // 0.16.0 > 0.15.0 numerically (components are ints, not strings) — see Fsgg.Version.
    ...
    let ``behind minimum emits exactly one cliBehindMinimum advisory naming installed minimum and gap`` () =
        ...
        | [ diagnostic ] ->
            ...
            Assert.Contains("0.15.0", diagnostic.Message)
